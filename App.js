...
// ─── MOCK INSTALLED APPS — replaced by react-native-installed-apps on device ──
const MOCK_APPS = [
  { packageName:'com.instagram.android',    appName:'Instagram', icon:'📸', locked:false, paused:false, inVault:false },
  { packageName:'com.google.android.youtube',appName:'YouTube',  icon:'▶️', locked:true,  paused:false, inVault:false },
  { packageName:'com.twitter.android',      appName:'X',         icon:'🐦', locked:true,  paused:false, inVault:false },
  { packageName:'com.spotify.music',        appName:'Spotify',   icon:'🎵', locked:false, paused:true,  inVault:false },
  { packageName:'com.zhiliaoapp.musically', appName:'TikTok',    icon:'🎬', locked:false, paused:false, inVault:true  },
  { packageName:'com.reddit.frontpage',     appName:'Reddit',    icon:'🔴', locked:true,  paused:false, inVault:false },
  { packageName:'com.discord',              appName:'Discord',   icon:'💬', locked:false, paused:true,  inVault:false },
  { packageName:'com.netflix.mediaclient', appName:'Netflix',    icon:'🎭', locked:false, paused:false, inVault:false },
  { packageName:'com.snapchat.android',    appName:'Snapchat',   icon:'👻', locked:false, paused:false, inVault:false },
  { packageName:'com.whatsapp',            appName:'WhatsApp',   icon:'💚', locked:false, paused:false, inVault:false },
  { packageName:'com.linkedin.android',    appName:'LinkedIn',   icon:'💼', locked:false, paused:false, inVault:false },
  { packageName:'com.facebook.katana',     appName:'Facebook',   icon:'👤', locked:false, paused:false, inVault:false },
  { packageName:'com.google.android.gm',   appName:'Gmail',      icon:'📧', locked:false, paused:false, inVault:false },
  { packageName:'com.ubercab',             appName:'Uber',        icon:'🚗', locked:false, paused:false, inVault:false },
  { packageName:'com.amazon.mShop',        appName:'Amazon',     icon:'📦', locked:false, paused:false, inVault:false },
];
...
// ─── APP LOCK OVERLAY ─────────────────────────────────────────────────────────
// Shown when Accessibility Service detects a locked app in foreground
function AppLockOverlay({ T, animLevel, app, pinHash, onUnlock, onCancel }) {
  const [mode, setMode] = useState('bio'); // 'bio' | 'pin'
  const { authenticate } = useAppLockAuth();

  async function tryBio() {
    const result = await authenticate(app?.appName || 'app');
  ...
  return (
    <View style={[StyleSheet.absoluteFill, styles.lockOverlay, { backgroundColor: T.bg }]}>
      <View style={styles.lockWrap}>
        ...
        <TouchableOpacity style={[styles.ghostBtn, { borderColor: T.accent }]} onPress={() => setMode('pin')}>
...
export default function App() {
  ...
  const [lockedApp, setLockedApp] = useState(null); // app being intercepted
  ...
  useEffect(() => {
    (async () => {
      ...
      // Load real installed apps if library available
      try {
        const InstalledApps = require('react-native-installed-apps').default;
        const apps = await InstalledApps.getApps();
        if (apps && apps.length > 0) {
          const mapped = apps.map(a => ({
            packageName: a.packageName,
            appName: a.label || a.appName,
            icon: '📱',
            locked: (saved?.installedApps || []).find(x => x.packageName === a.packageName)?.locked || false,
            paused: (saved?.installedApps || []).find(x => x.packageName === a.packageName)?.paused || false,
            inVault:(saved?.installedApps || []).find(x => x.packageName === a.packageName)?.inVault || false,
          }));
          dispatch({ type: 'LOAD', state: { ...saved, installedApps: mapped } });
        }
      } catch (e) {
        // Use mock apps in dev/snack
        console.log('[APP] Using mock apps:', e.message);
      }
    })();
  }, []);
  ...
  useEffect(() => {
    // Don't save pinHash here — it's in SecureStore
    const { pinHash, gestureEncoded, recoveryPhrase, ...toSave } = state;
    saveState(toSave);
  }, [state]);

  // AppState — lock on background
  useEffect(() => {
    const sub = AppState.addEventListener('change', next => {
      if (appStateRef.current === 'active' && next.match(/inactive|background/)) {
        // App going to background — lock vault, clear intercepted app
        setLockedApp(null);
      }
  ...
  return (
    <View style={[styles.root, { backgroundColor: T.bg }]}>
      ...
      {/* App lock overlay — shown when Accessibility Service intercepts a locked app */}
      {lockedApp ? (
        <AppLockOverlay
          T={T}
          animLevel={animLevel}
          ...
          onCancel={() => {
            setLockedApp(null);
            // Ideally navigate user away from the locked app — platform limitation
          }}
        />
      ) : null}
